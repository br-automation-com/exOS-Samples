...
void WaterTankDatamodel::connect() {
    ...
    FillValve.connect((EXOS_DATASET_TYPE)EXOS_DATASET_PUBLISH);
    EnableHeater.connect((EXOS_DATASET_TYPE)EXOS_DATASET_PUBLISH);
    HeaterConfig.connect((EXOS_DATASET_TYPE)(EXOS_DATASET_PUBLISH+EXOS_DATASET_SUBSCRIBE));
    Status.connect((EXOS_DATASET_TYPE)EXOS_DATASET_SUBSCRIBE);
}
